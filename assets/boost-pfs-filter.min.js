...
(function() {
	...
	var allSoldOut = false;
		...
		let saleLabel = '';
		if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && preOrder) {
				saleLabel = "Pre Order"
		} else if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && limitedEdition ) {
				saleLabel = boostPFSTemplate.limitedEditionLabelHtml.replace(/{{style}}/g, '');
		} else if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && newProduct) {
				saleLabel = "New";
				// saleLabel =  boostPFSTemplate.newLabelHtml.replace(/{{style}}/g, '');
		}else if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && secondsProduct) {
				saleLabel = "Seconds";
			...
			boostPFSThemeConfig.custom.show_sold_out_label  && specialOrder) {
				saleLabel = "Special Order";
		}
		else if (boostPFSThemeConfig.custom.show_sale_label &&
			...
			return e.name == boostPFSFilterConfig.general.optionColorName;
					...
					for (var i = 0; i < data.variants.length; i++) {    
						...
						var 	preAviliableQty = false;
							let variantID = 	data.variants[i].id
								if (variantID) {
