...
var boostPFSFilterConfig = {
	general: {
        ...
        optionColorName: 'colour',
		filterOptionColorId: 'pf_opt_colour',
    },
	selector: {
...
(function() {
	...
	function prepareShopifyData(data) {
        ...
        var currenciesSeting = boostPFSConfig.general.currencies;
        if (typeof currenciesSeting != 'undefined' && currenciesSeting.length > 1) {
            ...
            prices.forEach((priceKey) => {
                ...
                if (typeof data[currentPriceKey] !== 'undefined') {
                    data[priceKey] = data[currentPriceKey];
                }
            });
            data.variants.forEach((v) => {
                var currentPriceKey = 'price_' + currentCurrency;
                v['price'] = parseFloat(v[currentPriceKey]);
            });
        }
	...
	function buildPrice(data) {
		let differnceInPrice = data.compare_at_price_min > data.price_min; // Check a product is on sale
		var html = '';
	...
	function buildLabels(data, variantId, preOrderLabel) {
      	

		let tagList = data.tags
				// #719766932 Added in additional flags
				// Creating all the aditional product flags and setting them to false
				preOrder = preOrderLabel,
				limitedEdition = false,
				newProduct = false,
				secondsProduct = false,
				restocked = false,
				onSale = false,
				specialOrder = false,
                allSoldOut = false
				...
				tagList.forEach(element => {
					if (element === "special order") {
						specialOrder = true;
					} else if	(element === "limited edition") {
						limitedEdition = true;
					} else if(element === "new") {
						newProduct = true
					}else if(element === "seconds") {
						...
						onSale = true
					}else if(element === "restocked") {
						restocked = true
					}else if(element === "sold out") {
						allSoldOut = true
					}
				});

		// #719766932 adding in additional labels
        let productQty = data.variants.reduce((sum, variant) => {
            return sum+= variant.inventory_quantity;
        },0)
      	let isComingSoon = data.tags.includes("coming soon");
		// Build Sold out label
		let saleLabel = '';
		if (boostPFSThemeConfig.custom.show_sale_label && boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && preOrder) {
				saleLabel = "Pre Order"
		} else if (boostPFSThemeConfig.custom.show_sale_label && boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && limitedEdition && !isComingSoon ) {
				saleLabel = boostPFSTemplate.limitedEditionLabelHtml.replace(/{{style}}/g, '');
		} else if (boostPFSThemeConfig.custom.show_sale_label && boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && newProduct) {
				saleLabel = "New";
				// saleLabel =  boostPFSTemplate.newLabelHtml.replace(/{{style}}/g, '');
		}else if (boostPFSThemeConfig.custom.show_sale_label && boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && secondsProduct) {
				saleLabel = "Seconds";
		}else if (boostPFSThemeConfig.custom.show_sale_label && onSale && !soldOut) {
			...
			// }
			// saleLabel = boostPFSTemplate.saleLabelHtml.replace(/{{style}}/g, '');
			// saleLabel = boostPFSTemplate.saleLabelHtml.replace(/{{salePercent}}/g);
	
			// saleLabel = boostPFSTemplate.saleLabelHtml.replace(/{{salePercent}}/g, salePercent + '%');
			// Removed sales percentages because we no longer use them and having this on boost broke the collection pages
		} else if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label && !soldOut  && restocked) {
				saleLabel = "Restocked";
		}
		else if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label  && specialOrder && newProduct) {
				saleLabel = "New";
		}
		else if (boostPFSThemeConfig.custom.show_sale_label &&
			boostPFSThemeConfig.custom.show_sold_out_label && allSoldOut) {
				saleLabel = "Sold Out";
		}  

		// Build Sold out or coming soon label
        
        let isPreOrder = data.tags.includes('pre order');
		var soldOutLabel = '';
		if (boostPFSThemeConfig.custom.hasOwnProperty('show_sold_out_label') &&
			boostPFSThemeConfig.custom.show_sold_out_label && soldOut && !isComingSoon && !isPreOrder) {
			soldOutLabel = '<span class="soldOut">Sold Out</span>'
        } else if (isComingSoon){
          	soldOutLabel = '<span class="soldOut">Coming Soon</span>'
        } else if (isPreOrder) {
            soldOutLabel = boostPFSTemplate.preorderLabelHtml;
        }
    ...
    //         saleLabel = boostPFSTemplate.saleLabelHtml.replace(/{{salePercent}}/g, salePercent + '%');
      ...
      console.log(saleLabel)
		return isPreOrder ?  soldOutLabel : soldOutLabel + saleLabel;
	}
	...
	function getVariantData(variants,variantId) {
	  var result;
      if (variants) {
				result = variants.find(v => (v.id == variantId ));
      }
      return result;
  	}


  	function getVariantMetafield(metafields, key, variantId) {
	  var result;
      if (metafields && metafields.length > 0) {
				

		 var temp = metafields.find(m => (m.namespace == variantId && m.key == key));

        if (temp && temp.value && temp.value != "null") result = temp.value;
      }
  ...
  /* Expand Filter */
	...
	ProductGridItem.prototype.compileTemplate = function(data) {
		...
		// Get Template
        /* #boost-79199: Variants as Products */
		var itemHtml = buildVariantsItem(data, 'grid');
		// Add Custom class
	...
	ProductListItem.prototype.compileTemplate = function(data) {
		...
		// For List View
		/* #boost-79199: Variants as Products */
		var itemHtml = buildVariantsItem(data, 'list');
    ...
    /* #boost-79199: Variants as products */
    function buildVariantsItem(data, type) {
        var html = '';	
				var colorGroups = {};
        var colors = [];

		
        var option = 'option_' + boostPFSFilterConfig.general.optionColorName;

				// #734988613 Merging boost fix
					/* Start-Boost-84104 */
					var optionIndex = data.options_with_values.findIndex(function(e) {
						return e.name == boostPFSFilterConfig.general.optionColorName;
					});
					/* End-Boost-84104 */
        ...
        for (var i = 0; i < data.variants.length; i++) {      
            ...
            var color = variant[option];
						var specialOrder = false;


						// Check to see if any of the variants have preorder dates in them that are active
            ...
            if (color && colors.indexOf(color) === -1) {
                ...
                if (data.options.length > 1){
                	colorVariants = data.variants.filter((item)=>{
                		return item.option1 == color
                	});
   
                	colorVariants = colorVariants.map((item)=> item.id);

                }
                let preOrderLabel = false;
                for (var k = 0; k < data.variants.length; k++) {    
                	var 	preOrderDate;
                	var 	fulfilmentDate;
                	var 	preAviliableQty = false;
                		let variantID = 	data.variants[k].id
                		if (colorVariants.indexOf(variantID) >-1) {
                		 preAviliableQty = getVariantMetafield(data.metafields, 'Available_Qty', variantID)
                		 currentVariant = getVariantData(data.variants, variantID)
              				if (preAviliableQty > 0 ) {
              					preOrderLabel = true; 
                ...
                if (Globals.queryParams.hasOwnProperty(boostPFSFilterConfig.general.filterOptionColorId)) {        
                    if (Globals.queryParams[boostPFSFilterConfig.general.filterOptionColorId].indexOf(color) === -1) continue;
                }   
								...
								if (!specialOrder) {
										// 719766932 Sold Out check
								for (let index = 0; index < data.variants.length; index++) {
									if (data.variants[index].option1 === color) {
										let currentVariant = data.variants[index]
										if (color in colorGroups) {
											colorGroups[color].total = colorGroups[color].total + 1
											if (!currentVariant.available) {
												colorGroups[color].outOfStock = colorGroups[color].outOfStock + 1
											}
										}else {
											colorGroups[color] = {total: 1, outOfStock: 0} 
											if (!currentVariant.available) {
												colorGroups[color].outOfStock =  1
											}
										}	
									}
								}
                ...
                if (!variant.available) {
                    var avalvar = data.variants.find(function(v) { return  v[option] === color && v.available;});
                    if (avalvar) variant = avalvar;
                }

                //if (!variant.available) continue;
                /* Start-Boost-84104 */
                if (boostPFSConfig.settings.general.productAndVariantAvailable && !variant.available) continue;
                /* End-Boost-84104 */

                // Get Template
                var itemHtml = type === 'grid' ? boostPFSTemplate.productGridItemHtml : boostPFSTemplate.productListItemHtml;
                ...
                onSale = variant.compare_at_price > variant.price;
								...
								// 719766932 SOLD OUT CHECK
								if (typeof colorGroups[variant.option1] != 'undefined') {
                ...
                itemHtml = itemHtml.replace(/{{saleClass}}/g, saleClass);
                // Add Label
                itemHtml = itemHtml.replace(/{{itemLabels}}/g, buildLabels(data, variant.id, preOrderLabel));              

                // Images
                var imageIndex = -1;
              	if (variant.image) {
                  imageIndex = data.images_info.findIndex(function(image) { return image.src === variant.image; });
                  if (imageIndex > -1) {
                      images = [data.images_info[imageIndex]];
                  }
                } else {
                  var value = Utils.slugify(color);
                  imageIndex = data.images_info.findIndex(function(img) { return img.src.indexOf(value) > -1; });
                  if (imageIndex > -1) {
                      images = [data.images_info[imageIndex]];
                  }
                }
                ...
                if (imageIndex > -1) {
                  if (typeof data.images_info[imageIndex + 1] !== 'undefined') {
                    images.push(data.images_info[imageIndex + 1]);
                  } else {
                    images.push(data.images_info[imageIndex]);
                  }
                }
                ...
                var priceData = {
                  ...
                  compare_at_price_min: variant.compare_at_price
   		        ...
   		        itemHtml = itemHtml.replace(/{{itemPrice}}/g, buildPrice(priceData));

                itemHtml = itemHtml.replace(/{{itemUrl}}/g, Utils.buildProductItemUrl(data) + '?variant=' + variant.id);
								itemHtml = itemHtml.replace(/{{itemUrl}}/g, Utils.buildProductItemUrl(data) + '?variant=' + variant.id);
								//itemHtml = itemHtml.replace(/{{itemTitle}}/g, data.title + ' - ' + Utils.capitalize(color));
											/* Start-Boost-84104 */
							var variantTitle = data.title + ' - ' + variant.title.split('/')[optionIndex];
        ...
        if (!html) html = type === 'grid' ? boostPFSTemplate.productGridItemHtml : boostPFSTemplate.productListItemHtml;
